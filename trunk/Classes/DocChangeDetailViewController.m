...
- (void) sendFileStoped:(FTP_ERROR)ftpError{
    ...
    [alert hideWaiting];
    [alert alertInfo:[NSString stringWithFormat:@"%@/n在上传时失败。",[attachArray objectAtIndex:fileCount]] withTitle:@"错误"];
}
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else if (alertType == ALERTTABLE_LEVEL)
    {
        ...
        if(cell != nil) {
            ...
            if ([fileManager fileExistsAtPath:filePath]&&[attachID isEqualToString:kAttachID_Invalide]) {
...
//                [alert showWaitingWithTitle:@"文件加载中" andMessage:@"请等待..."];
                ...
                ftpDownload = [[FTPDownloadFile alloc] initWithServerPath:[appDelegate.ftpInfo.ftpURL stringByAppendingFormat:@"/%@", fileName]
                                                                ...
                                                                withPass:appDelegate.ftpInfo.ftpPassword];
...
-(void) showMediaWithFile:(NSString *) fileName1{
    NSLog(@"------%@",fileName1);
    NSString *fileType = [fileName1 substringWithRange:NSMakeRange(0, 5)];
...
- (void)viewDidLoad {
    ...
    if (([info.recuseuserID isEqualToString: appDelegate.loginId])&& 
        !([contributeInfo.status isEqualToString:@"3"] || 
          [contributeInfo.status isEqualToString:@"5"])) {
        enableAudit = YES;
    }else{
        enableAudit = NO;
    }
